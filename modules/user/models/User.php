...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        if (isset(static::$rolesTitles[$role])) {
            return static::$rolesTitles[$role];
        }
        return Yii::t('*', 'Неизвестная роль');
    }
    ...
    public function setRole($role, $active = true)
    {
    ...
    public function checkRole($role, $strict = false)
    {
    ...
    {
        $roles = [];
        foreach (static::$rolesTitles as $role => $title) {
            if (in_array($role, static::$availableRoles) && $this->checkRole($role)) {
                $roles[$role] = $title;
            }
        }
        return $roles;
